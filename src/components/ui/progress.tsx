      ...
      >
        {(label || !isIndeterminate) && (
          <div className="flex justify-between items-center font-label text-sm font-semibold text-on-surface">
            ...
            {!isIndeterminate && (
              <BaseProgress.Value className="font-mono text-sm font-bold text-on-surface">
        ...
        <BaseProgress.Track className="relative w-full h-3 rounded-full bg-surface-variant overflow-hidden border border-outline-variant/30">
          <BaseProgress.Indicator
            className={cn(
              'h-full bg-primary transition-[width] duration-[var(--duration-medium)] ease-[var(--ease-standard)]',
