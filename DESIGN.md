...
- **Floating Containers:** Dialog, Drawer, Popover, Menu/Select/Dropdown, and Toast are elevated above page content with dedicated permanent shadows and consistent tier-based geometry:
  - **Tier A — Modal, blocking (Dialog, Drawer):** `rounded.lg` (1rem / 16px), `2px` border (`borderWidth.emphasis`), modal elevation (`shadows.modal` / `shadow-modal`) + backdrop scrim behind the container.
...
- **`rounded.lg` (1rem):** Standard Cards, Dialogs, and non-docked edges of Drawers (e.g. `rounded-l-lg` for right-side drawers, `rounded-t-lg` for bottom sheets). Docked edges touching the viewport bezel remain flush (`rounded-none`).
...
**Dialog and Drawer (Tier A — modal, blocking):** a `surface` panel with `1rem` corner radius (`rounded.lg`), a `2px` `outline-variant` border (`borderWidth.emphasis`), modal elevation (`shadows.modal` / `shadow-modal`), and a backdrop scrim dimming the page behind them (see Elevation & Depth). Reserved for tasks that must interrupt the flow — destructive confirmations, focused single-step forms. Prefer a side **Drawer** over a full modal for longer or multi-field tasks so users keep page context. Drawers carry the same weight as Dialogs (modal and blocking) but anchor to a screen edge, docking flush against the viewport bezel: clear corner radii (`rounded-none`) and remove borders along docked edges (e.g., `rounded-l-lg rounded-r-none border-l-[2px] border-y-0 border-r-0` on a right-side drawer, or `rounded-t-lg rounded-b-none border-t-[2px] border-x-0 border-b-0` on a bottom sheet) to prevent bezel lines and crescent cutouts revealing the backdrop scrim.

**Popover (Tier B — anchored, non-modal):** a `surface` panel with `0.5rem` corner radius (`rounded.DEFAULT`, matching Menus, Selects, and Comboboxes), the lighter standard `1px` `outline-variant` border (`borderWidth.default`), and ambient elevation (`shadows.ambient` / `shadow-ambient`) — no backdrop. Lightweight and transient, dismisses on outside click or `Escape`. Never use a Popover to host a primary task's main content — that's what Dialog/Drawer are for.
